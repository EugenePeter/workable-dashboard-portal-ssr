...
const MainContainer: React.FC<any> = () => {
  ...
  const storedActiveTab = JSON.parse(localStorage.getItem("active_tab"));

  const data: Idata = {
    tab_data,
    // active_tab,
    // tabs,
    active_tab: storedActiveTab ? storedActiveTab : active_tab,
    tabs: storedTabs ? storedTabs : tabs,
  };

  console.log("NAAY SUD:", data);

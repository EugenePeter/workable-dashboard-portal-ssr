              ...
              items.map((item: string, index: number) => (
                <DropDown key={index}>
                  ...
                  <DropDownItems
                    ...
                    onClick={(event) => handleInputChange({ event, name, item })}
                    key={index}
                  >
                    {item}
                  </DropDownItems>
              ...
              items.map((item: string, index: number) => (
                <>
                  <CarouselButton
                    ...
                    className={
                      is_selected && index === is_selected.index ? `carousel-button__selected` : ""
                    }
                  >
                    {item}
                  </CarouselButton>
...
export const Pointer = styled.div<CarouselButtonWrapperProps>`
  ...
  width: 16px;
  height: 16px;
  transform: ${(props) => (props.pointer === "left" ? "rotate(-134deg)" : "rotate(45deg)")};
`;
...
export const Label = styled.label<LabelProps>`
  ...
  transform: ${({ is_input_active }) =>
    is_input_active ? `translate(.8rem, -.7rem)` : `translate(1rem, .4rem)`};
  transition: transform 0.4s cubic-bezier(0.23, 0, 0, 1.01);
  &:hover {
    cursor: text;
  }
